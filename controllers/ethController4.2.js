...
async function getTransactionListByRange(address, from, count) {
    try {
        if (!utils.isAddress(address))
            throw new Error('Address not valid in Ethereum');
        if (from < 0)
        ...
        let transactionList = await ethTransaction.getTransactionListByRange(address, from, count);
        let transactionPending = await ethTransaction.getPendingTxs(address);
        return {
            'pending' : transactionPending ,
            'transactions': transactionList
...
async function sendRawTransaction(rawTransaction) {
    try {
        ...
        let transactionInfo = await getTransactionFromHash(transactionHash);
        ethTransaction.saveTempTransaction(transactionInfo);
