...
BitcoinDb.prototype.getBalanceFast = async function (address) {
    ...
    txs.forEach(tx => {
        if (tx.vin) tx.vin.forEach(tvin => {
            if (this._checkVin(tvin, address)) outcome += tvin.value;
        });
        tx.vout.forEach(tvout => {
            if (this._checkVout(tvout, address)) income += tvout.value;
        })
    });
    return income - outcome;
};
BitcoinDb.prototype.getTransactionsCount = async function (address) {
...
BitcoinDb.prototype._checkVin = function (vin, address) {
    return (vin &&
        vin.addresses &&
