...
module.exports = class BitKind extends BitKindRpc {
    ...
    async scan(from, to, updateRange) {
        console.log('Start scan');
        this.saveBlockTransactionFromTo(from, to, 10, function (lastBlock) {
            updateRange(lastBlock);
        })
    }
    async checkBadBlocks(order, finishMethod) {
        const taskQue = new Queue(order);
        let badBlocks = await ScanLogModel.getLogs(this.blockchain);
        if (badBlocks) {
            await Promise.all(badBlocks.map(async (element) => {
                try {
                    taskQue.pushTask(async done => {
                        let blockData = await this.getTransactionsFromBlock(element.blockNum);
                        if (blockData) {
                            await Promise.all(blockData.map(async (tr) => {
                                this.transactionLib.saveTransactionToMongoDb(tr)
                                    .then(() => {
                                        ScanLogModel.setStatusTrueLogByBlockId(tr.blockheight);
                                        console.log(`Bad BlockNum: ${tr.blockheight}`);
