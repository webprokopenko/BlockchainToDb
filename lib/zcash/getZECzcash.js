const zecConfig = require(`../../config/config.json`).ZECRpc,
    sendRPC = require('../../lib/utils').sendRPC;
...
function sendRawTransaction(hex) {
    return new Promise((resolve, reject) =>{
        hex = String(hex) || '';
        sendRPC('sendrawtransaction', zecConfig, [hex])
            .then(resp => {
                try {
...
function getBlockCount() {
    return new Promise((resolve, reject) =>{
        sendRPC('getblockcount', zecConfig)
            .then(block => {
...
function getBlockData(hash) {
    return new Promise((resolve, reject) => {
        try{
            let blockM = {};
            sendRPC('getblock', zecConfig,[hash, true])
                .then(blockData => {
...
function getBlockHash(numBlock) {
    return new Promise((resolve, reject) =>{
        sendRPC('getblockhash', zecConfig, [numBlock])
            .then(block => {
                let jblock = JSON.parse(block);
                if(block === undefined)
...
function getTransactionById(id) {
    return new Promise((resolve, reject) => {
        sendRPC('getrawtransaction', zecConfig, [id, 1])
            .then(resp => {
                if(resp === undefined)
