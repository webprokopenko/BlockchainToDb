...
async function getTransactionListByRange(address, from , count = 50){
    return new Promise((resolve, reject) => {
        try {
            BlockTransaction
                .find({ $or:[ {'from':address}, {'to':address} ]})
                ...
                .sort({timestamp: -1})
                .then(res => {
                    res ? resolve(res) : reject(new Error(`function getTransactionListByRange no block number`));
                })
        } catch (error) {
            reject(error);
        }
    ...
    return new Promise((resolve, reject) => {
        try {
            BlockTransaction
                ...
                .select('-_id -blockNum -__v')
                .then(res => {
                    res ? resolve(res) : reject(new Error(`function getContractTransfers no block number`));
                })
        } catch (error) {
            reject(error);
        }
...
async function getContractTransfersCount(contractAddress, address) {
    return new Promise((resolve, reject) => {
        try {
            BlockTransaction
                .count({ $or:[
    ...
    return new Promise((resolve, reject) => {
        try {
            BlockTransaction
                ...
                .limit(1)
                .then(res => {
                    res && res.length ? resolve(res[0].blockNum) : resolve(0);
                })
                .catch(e => reject(e));
        } catch (error) {
            reject(error);
...
module.exports = {
    ...
    getContractTransfersCount:      getContractTransfersCount,
