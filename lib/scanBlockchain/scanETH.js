...
async function getTransactionFromETH(numBlock) {
    try {
        ...
        await Promise.all(blockData.transactions.map(async (element) => {
            ...
            let inputValue = transaction.input.value ? Units.convert(math.bignumber(utils.convertHexToInt(transaction.input.value)).toFixed(), 'wei', 'eth').toString() : '0';
            transaction.input.value = inputValue;
...
async function scanIndexer(){
    try {
        let from = await dbEthertransactionsLib.getLastMongoBlock();
        let to = await gethRPC.getBlockNumber();
        scan(from, to, (lastBlock) => {
            console.log('Scan Indexer return lastBlock: ' + lastBlock);
            (to - from < 100)?checkBadBlocks(10, ()=>{
                console.log('checkBadBlocks done!!!');
            }):null
        }) 
    } catch (error) {
...
async function saveBlockTransactionFromTo(from, to, order, finishMethod, clearTemp=true) {
    try {
                ...
                if (blockData) {
                    try {
                        await Promise.all(blockData.map(async (element) => {
                            ...
                            if(clearTemp) dbEthertransactionsLib.removeTempTansaction(element.hash);
                        }));
                    } catch (error) {
...
async function checkBadBlocks(order, finishMethod) {
    console.log('Start checkBadBlocks');
    const taskQue = new Quequ(order);
