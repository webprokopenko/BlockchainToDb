                ...
                getTxs(jblock.result.tx_hashes)
                    .then(txs => {
                        try {
                            let tx = {};
                            let i = 0;
                            return resolve(txs.map(t => {
                                tx = Object.assign({
                                    output_indices: t.output_indices,
                                    block_height:   t.block_height,
                                    block_timestamp:t.block_timestamp,
                                    hash:           t.tx_hash
                                },t.as_json);//console.dir(tx);console.log(i++);
                                return {
...
function _sendAPI(data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            xhr.onerror = () => {
                XmrError.error(`${new Date()} monero node error ${xhr.responseText}`);
                return reject('Service error');
            };
            xhr.send(data.data?JSON.stringify(data.data):null);
        } catch (err) {
            XmrError.error(`${new Date()} monero sendAPI error ${err}`);
            return reject('Service error');
        }
    })
}
