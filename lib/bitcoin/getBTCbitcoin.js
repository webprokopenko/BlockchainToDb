    ...
    return new Promise((resolve, reject) => {
        try {
            if(!Utils.isAddress(address, btcConfig.network))
                return resolve({error: 'getTxsByAddress error: incorrect address - ' + address});
            BTCTransaction.find()
                .where(
                    {'vout.scriptPubKey.addresses': address}
                )
                .then(txs => {
                    return resolve({txs: txs.map(tx => {
                        return {
...
async function getUTXOs(address) {
    try {
        if(!Utils.isAddress(address, btcConfig.network))
            return {error: 'getTxsByAddress error: incorrect address - ' + address};
        let tx = {};
...
async function getBalance(address) {
        try {
            if(!Utils.isAddress(address, btcConfig.network))
                return {error: 'getBalance error: incorrect address - ' + address};
            let tx = {};
            const txs = await _getVoutTxsByAddress(address);
            let income = 0;
            let outcome = 0;
        ...
        _sendAPI(params)
            .then(resp => {
                ...
                return resolve(res.result ?
                    {txid: res.result} : {error: res.error});
            })
            .catch(err => {
