...
function getBalanceCmd(addr){
    return new Promise((resolve, reject) => {
    clientBtc.cmd('getbalance', addr, 1, true, function(err, balance){
        if (err) 
...
function sendRawTransacitonCmd(raw){
    return new Promise((resolve, reject) => {
    clientBtc.cmd('sendrawtransaction', raw, function(err, res){
        if (err) return reject(new Error(`getBTCbitcoin sendRawTransacitonCmd error ${err}`));
        resolve(res);
      });
    });  
}
function listUnspentCmd(addr){
    return new Promise((resolve, reject) => {
        clientBtc.cmd('listUnspent', 1, 1, addr, function(err, res){
            if (err) return reject(new Error(`getBTCbitcoin listUnspentCmd error ${err}`));
            resolve(res);
        });
    });
}
function gettxoutCmd(txid){
    return new Promise((resolve, reject) => {
        clientBtc.cmd('gettxout', 1, false, txid, function(err, res){
            if (err) return reject(new Error(`getBTCbitcoin gettxoutCmd error ${err}`));
            resolve(res);
        });
    });
} 
