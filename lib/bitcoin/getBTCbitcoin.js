    ...
    try {
        ...
        for(let txI = 0; txI < txs.length; txI++){
            ...
            if (tx.vout[0].scriptPubKey.addresses &&
                tx.vout[0].scriptPubKey.addresses.indexOf(address) >= 0) {
                first = true;
            }
            if (tx.vout[1].scriptPubKey.addresses &&
                tx.vout[1].scriptPubKey.addresses.indexOf(address) >= 0) {
                second = true;
            }
                    ...
                    for(let tvii = 0; tvii < tvin.vin.length; tvii++) {
                        if (tx.txid === tvin.vin[tvii].txid) {
                            if(first && tvin.vin[tvii].vout === 0) {
                                unspent = false;
                                _pushUTXO(utxos, tx, 0, address);
                            }
                            if(second && tvin.vin[tvii].vout === 1) {
                                unspent = false;
                                _pushUTXO(utxos, tx, 1, address);
                            }
        ...
        return new rpcError('getUTXOs error: ' + error, 'btc', 202);
    }
}
...
async function getBalance(address) {
    try {
        ...
        return new rpcError('getBalance error: ' + error, 'btc', 202);
    }
}
        ...
        sendRPC('sendrawtransaction', btcConfig, [hex])
            .then(resp => {
                try {
                    ...
                    if(res.error) {
                        return reject(new rpcError('Code-114 sendrawtransaction error: ' + res.error.message, 'btc', res.error.code));
                    }
                    if (!res.result)
    ...
    return new Promise((resolve, reject) =>{
        sendRPC('getblockcount', btcConfig)
            .then(block => {
                ...
                if(jblock.error)
                    return reject( new rpcError(`RPC getBlockCount return error ${jblock.error}`, 'btc', jblock.error.code));
                if(!jblock.result)
                    return reject( new rpcError(`RPC getBlockCount return no result`, 'btc', 200));
    ...
    return new Promise((resolve, reject) =>{
        sendRPC('getblock', btcConfig, [hash, 2])
            .then(block => {
                let jblock = JSON.parse(block);
                if(jblock.error)
                    return reject( new rpcError(`RPC getblock return error ${block.error}`, 'btc', jblock.error.code));
                if(!jblock.result)
                    return reject( new rpcError(`RPC getblock return empty result by hash ${hash}`, 'btc', 200));
                return resolve(jblock.result);
            })
            .catch(err => {
