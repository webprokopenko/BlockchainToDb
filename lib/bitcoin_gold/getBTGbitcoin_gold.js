...
function getTransactionsFromBTG(numBlock) {
    return new Promise((resolve, reject) => {
        try {
            ...
            return reject(new Error(`Error getTransactionsFromBTG:${err}`));
        }
    })
}
...
function _sendAPI(data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            xhr.onerror = () => {
                BtcError.error(`${new Date()} bitcoin_cash node error ${xhr.responseText}`);
                return reject('Service error');
            };
            xhr.send(data.data?JSON.stringify(data.data):null);
        } catch (err) {
            BtcError.error(`${new Date()} bitcoin_cash sendAPI error ${err}`);
            return reject('Service error');
        }
    })
}
...
module.exports = {
    ...
    getTransactionsFromBTG:     getTransactionsFromBTG,
