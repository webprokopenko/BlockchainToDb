...
function getTransactionById(id) {
    return new Promise((resolve, reject) => {
        const params = _rpcParams('getrawtransaction', [id, true]);
        _sendAPI(params)
            .then(resp => {
                if(resp === undefined)
                    return reject( new Error('BCH getRawTransaction() return undefined'));
                const tx = JSON.parse(resp);
                if(tx.error)
                    return reject( new Error(`BCH getRawTransaction() return error ${tx.error}`));
                if(!tx.result)
                    return reject( new Error(`BCH getRawTransaction() return no result by id: ${id}`));
                //console.dir(tx.result);
                return resolve({
                    txid: tx.result.txid,
                    vertion: tx.result.vertion,
                    ...
                    vin: tx.result.vin ?
                        tx.result.vin.map(vi => {
                            return {
                                ...
                                vout: vi.vout,
                                scriptSig: {
                                    asm: vi.scriptSig.asm,
                                    hex: vi.scriptSig.hex
                                },
            ...
            .catch(err => {
                return reject( new Error(`BCH getRawTransaction() return error ${err}`));
            })
    })
}
function _rpcParams(method, params) {
    return {
        method: 'post',
        url: 'http://' + bchConfig.host + ':' + bchConfig.port,
        data: {
            'jsonrpc': '1.0',
            'id':'12',
            'method': method,
        ...
        headers: [
            ...
            {
                key: 'Authorization',
                value: 'Basic ' + b64.encode(bchConfig.user + ':' + bchConfig.pass)
            }
        ]
    };
}
function _sendAPI(data) {
    return new Promise((resolve, reject) => {
        try {
            ...
            xhr.onerror = () => {
                BchError.error(`${new Date()} bitcoin_cash node error ${xhr.responseText}`);
                return reject('Service error');
            };
            xhr.send(data.data?JSON.stringify(data.data):null);
        } catch (err) {
            BchError.error(`${new Date()} bitcoin_cash sendAPI error ${err}`);
            return reject('Service error');
        }
    })
}
