            ...
            sendRPC('getblock', bchConfig, [hash, true])
                .then(blockData => {
                    ...
                    if(!block.result || !block.result.tx)
                        return reject( new Error(`RPC getblock return no result by hash ${hash}`));
                    blockM = block.result;
                    return _getBlockTxsJson(blockM.tx);
                })
                .then(txs => {
                    if(!txs) {
                        return reject(new Error(`RPC getblock
                ...
                .catch(err => {
                    return reject( new Error(`RPC getblock return error ${err}`));
                });
        } catch  (error) {
            return reject( new Error(`RPC getblock return error ${error}`));
        }
    })
}
