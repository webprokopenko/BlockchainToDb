...
const isContractTransfer = function (tx) {
    try {
        if(tx.input
            && isString(tx.input)
            ...
            data.value = toBigNumber('0x' + tx.input.substr(74,64)).toString();
            return data;
        }
        return false;
    } catch (error) {
...
const isContractTransferFrom = function (tx) {
    try {
        if(tx.input
            && isString(tx.input)
            ...
            data.value = toBigNumber('0x' + tx.input.substr(138,64)).toString();
            return data;
        }
        return false;
    } catch (error) {
...
module.exports = {
    ...
    isAddressNBTT:          isAddress,
    isChecksumAddress:      isChecksumAddress,
    padLeft:                padLeft,
    toTwosComplement:       toTwosComplement,
    isHash:                 isHash,
    isContractTransfer:     isContractTransfer,
    isContractTransferFrom: isContractTransferFrom
}