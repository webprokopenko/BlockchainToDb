        ...
        try {
            ...
            sendRPC('eth_call', RpcConnectOption, [
                ...
                .then(resp => {
                    ...
                    if(!res)
                        return reject(new Error('RPC eth_call return bad response.'));
                    if(res.error)
                        return reject(new Error('Error Code-114.'
                        + res.error.message));
                    if(!res.result|| !utils.isString(res.result))
                        return reject(new Error('RPC eth_call return bad result.'));

                    return resolve(utils
                        .toBigNumber(res.result).toString())
                })
                .catch(err => {
                    return reject(new Error('RPC eth_call Error:' + err));
                })
        } catch (error) {
