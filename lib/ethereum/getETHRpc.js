...
function getClearBlockData(numBlock) {
    return new Promise((resolve, reject) => {
        sendRPC('eth_getBlockByNumber', RpcConnectOption, [utils.convertNumberToHex(numBlock), true])
            .then(resp => {
                const res = JSON.parse(resp);
                ...
                res.result.transactions.forEach(tx => {
                    tx.isContract = _isContractTransferFrom(tx);
                });
...
function getCode(address) {
    return new Promise((resolve, reject) => {
        sendRPC('eth_getCode', RpcConnectOption, [address, '0x2'])
            .then(resp => {
                const res = JSON.parse(resp);
                if (res.error)
                    return reject(new rpcError(`getCode Error from command geth  Message: ${res.error.message}`, 'eth', res.error.code));
                if (!res || !res.result)
                    return reject(new rpcError('getCode response body empty', 'eth', 200));

                resolve(res.result);
            })
            .catch(err => {
                return reject(new rpcError(`getCode Error from geth: ${err}`, 'eth', 202));
            })
    })
}
...
module.exports = {
    ...
    getClearBlockData:  getClearBlockData,
    getCode:    getCode,
