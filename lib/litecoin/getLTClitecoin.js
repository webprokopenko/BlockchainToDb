...
async function getUTXOs(address) {
    try {
        ...
        throw new rpcError('getUTXOs error: ' + error, 'ltc', 202);
    }
}
/**
...
async function getBalanceNew(address) {
    let income = 0;
    let outcome = 0;
    const txs = await LTCTransaction.find()
        .where(
            {$or: [
                    {'vin.addresses': address},
                    {'vout.scriptPubKey.addresses': address}
...
async function getTransactionsFromBlock(numBlock) {
    try {
        ...
        const txs = [];
        if (!blockData.tx) {
            throw new rpcError(`No data in block:${numBlock}`, 'ltc', 200);
        }
        ...
        throw new rpcError(`Error getTransactionsFromBlock: ${err}`, 'ltc', 203);
    }
}
