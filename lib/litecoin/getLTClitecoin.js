const ltcConfig = require(`../../config/config.json`).LTCRpc,
    ...
    Utils = require('../../lib/litecoin/utilsLTC');
    ...
    return new Promise((resolve, reject) => {
        try {
            LTCTransaction.find()
                ...
                .catch(err => {
                    return reject(new Error('getTxsByAddress error: ' + error));
                });
        } catch (err) {
            return reject(new Error('getTxsByAddress error: ' + error));
        }
        ...
        _sendAPI(params)
            .then(resp => {
                try {
                    ...
                    if(res.error) {
                        return reject(new Error('Send raw transaction error: ' + res.error));
                    }
...
function getTransactionsFromLTC(numBlock) {
    return new Promise((resolve, reject) => {
        try {
...
function _rpcParams(method, params) {
    return {
        method: 'post',
        url: 'http://' + ltcConfig.host + ':' + ltcConfig.port,
        data: {
            'jsonrpc': '1.0',
            'id':'12',
            'method': method,
        ...
        headers: [
            ...
            {
                key: 'Authorization',
                value: 'Basic ' + b64.encode(ltcConfig.user + ':' + ltcConfig.pass)
            }
        ]
    };
}
...
module.exports = {
    ...
    getTransactionsFromLTC:     getTransactionsFromLTC,
